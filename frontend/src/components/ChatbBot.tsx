        ...
        const fetchHistory = async () => {
            if (!token) { 
                ...
                setMessages([{ 
                    id: 0, 
                    sender: 'ai', 
                    text: "Please log in to use the advisor.",
            ...
            try {
                ...
                setMessages([{ 
                    id: 0, 
                    sender: 'ai', 
                    text: "Sorry, I couldn't connect to the advisor. Please try again.",
                    timestamp: new Date().toISOString()
                }]);
            } finally {
                setIsLoading(false);
                        ...
                        <div className="flex items-end justify-start">
                            <div className="px-4 py-3 rounded-2xl max-w-sm md:max-w-md bg-slate-700 text-slate-200 rounded-bl-none">
                                <div className="flex items-center space-x-2">
                                    <span className="h-2 w-2 bg-slate-400 rounded-full animate-pulse [animation-range:-0.3s]"></span>
                                    <span className="h-2 w-2 bg-slate-400 rounded-full animate-pulse [animation-range:-0.15s]"></span>
